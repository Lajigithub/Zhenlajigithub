...
def zhuanpan():
   print('\n【13-14】')
   try:
       bd=f'''------WebKitFormBoundary8GF8jBz3hbSaZqTm--'''
       
       
       msg=''
       hd_={"Accept": "application/json, text/plain, */*","Accept-Encoding": "gzip, deflate, br","Accept-Language": "zh-cn","Connection": "keep-alive","Content-Type": "multipart/form-data; boundary=----WebKitFormBoundary8GF8jBz3hbSaZqTm","User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_4_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148/zealer/3.1.1","accessToken": hd['accesstoken']}
       hd_['vesionCode']=hd['vesionCode']
       ...
       if res['status']==200:
          ...
          if res['data']['surplus_num']>0:
              ...
              bd=f'''--Boundary+A80FA168BC4A7504
Content-Disposition: form-data; name="template_id"

1
--Boundary+A80FA168BC4A7504
Content-Disposition: form-data; name="aid"

0
--Boundary+A80FA168BC4A7504--'''
              hd_['Content-Type']=f'''multipart/form-data; boundary='''+bd.split('\n')[0][2:]
              while True:
...
def HDZC_jojo(url,aid):
   print('\n【10-11-12】')
   try:
       bd=f"aid={aid}&verifyStr=e7042f0698b43dd3ff5eacfde073064d&verifyTime=1622029597"
...
def havingenergy(energyId):
       ...
       bd=f'''--Boundary+EFA955D29C1DB58E
Content-Disposition: form-data; name="energyId"

{energyId}
--Boundary+EFA955D29C1DB58E--'''
       
       hd_={"Accept": "application/json, text/plain, */*","Accept-Encoding": "gzip, deflate, br","Accept-Language": "zh-cn","Connection": "keep-alive","Content-Type":"multipart/form-data; boundary=Boundary+EFA955D29C1DB58E","User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_4_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148/zealer/3.1.1","accessToken": hd['accesstoken']}
       hd_['vesionCode']=hd['vesionCode']
...
def openbox():
   print('\n【8】')
   try:
       bd=f'''------WebKitFormBoundaryGo2cnB5bKPs5Dhto--'''
       msg=''
       hd_={"Accept": "application/json, text/plain, */*","Accept-Encoding": "gzip, deflate, br","Accept-Language": "zh-cn","Connection": "keep-alive","Content-Type": "multipart/form-data; boundary=----WebKitFormBoundaryf0SB8zu6PSyjeMyM","User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 14_4_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148/zealer/3.1.1","accessToken": hd['accesstoken']}
       hd_['vesionCode']=hd['vesionCode']
...
4
------WebKitFormBoundaryGo2cnB5bKPs5Dhto--'''
         ...
         if s==2:
           ...
           msg+=f"{res['data']['userInfo']['nickname']}|{res['data']['userInfo']['energy']}|{res['data']['userInfo']['score']}\n"
            ...
            if res['data']['boxWaitingOpen']==1:
             openbox()
            energyList=res['data']['energyList']
            print(energyList)
            
            for data in energyList:
               if data['id']!=-1:
                  print(data['id'])
                  if havingenergy(data['id']):
                     print('收取'+data['name']+str(data['energy'])+'点')
                  else:
                     print('能量收取失败')
                     msg+='【能量收取失败❌】\n'
